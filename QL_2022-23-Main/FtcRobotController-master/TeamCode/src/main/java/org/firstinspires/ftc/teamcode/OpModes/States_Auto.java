...
public class States_Auto extends LinearOpMode {
    ...
    public Pose2d PRE_LOAD_DEPOSIT = new Pose2d(9.5, 60, Math.toRadians(36.5));
    public Pose2d PRE_LOAD_DEPOSIT_FORWARD = new Pose2d(2, 53, Math.toRadians(90));
    ...
    public Pose2d DEPOSIT_HIGH_FAR_CLEAR = new Pose2d(18, 54, Math.toRadians(90));
    ...
    public static Pose2d DEPOSIT_HIGH_FAR = new Pose2d(33.5, 42.75, Math.toRadians(121)); //SECOND HIGH
    public static Pose2d DEPOSIT_HIGH_FAR_FORWARD = new Pose2d(33, 46, Math.toRadians(121)); //SECOND HIGH
    ...
    double slideHeightOne = 110;
    double slideHeightTwo = 95;
    double slideHeightThree = 60;
    double slideHeightFour = 45;

    int depositHeightPreload = 530;
    int depositHeightMid = 340;
    int depositHeightFarHigh = 530;
        ...
        while (opModeIsActive()) {
            ...
            switch (mRobotState) {
                ...
                case PRELOAD_DEPOSIT:
                    ...
                    if(time.time() > 0.2 && time.time() < 0.45) {
                        robot.arm.GrabberDeposit();
                    }

                    if(time.time() > 0.45 && time.time() < 0.55){
                        robot.arm.GrabberClose();
                    }

                    if(time.time() > 0.55 && time.time() < 1.15){
                ...
                case DRIVE_TO_INTAKE:
                    if(isDown && robot.localizer.getPose().getX() > -10) {
                    ...
                    if(Math.abs(robot.getPos().getX()) > 10){
                        ...
                        robot.arm.GrabberOpen();
                        robot.drive.followLine(true, 20, VisionConstants.LineFollowerTarget, robot.getPos().getX(), LineFollower.midMaxPoint.x, 0.5, 0.75);
                    }

                    break;
                case GRAB:
                    if (cycle == 0) {
                        robot.slides.setPosition(slideHeightOne - 10, -0.3, 1);
                    } else if (cycle == 1) {
                        robot.slides.setPosition(slideHeightTwo - 10, -0.3, 1);
                    } else if (cycle == 2) {
                        robot.slides.setPosition(slideHeightThree - 10, -0.3, 1);
                    }else if (cycle == 3){
                        robot.slides.setPosition(slideHeightFour - 10, -0.3, 1);
                    }
                ...
                case DRIVE_TO_DEPOSIT_HIGH_FAR:
                    ...
                    if(robot.getPos().vec().distTo(points.get(points.size() - 1).toVec()) < 27.5) {
                        if(cycle == 2){
                    ...
                    if(robot.getPos().vec().distTo(points.get(points.size() - 1).toVec()) < 1.2) {
                        newState(State.DEPOSIT);
                    }
                    break;
